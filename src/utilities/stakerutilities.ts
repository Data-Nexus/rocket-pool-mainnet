...
import {NetworkStakerBalanceCheckpoint, Staker, RocketETHTransaction, RocketPoolProtocol} from "../../generated/schema";
...
class StakerUtilities {
   ...
   */
  public hasNetworkStakerBalanceCheckpointHasBeenIndexed(protocol: RocketPoolProtocol, event: ethereum.Event): boolean {
    // If this specific event has been handled, then return true.
    if (NetworkStakerBalanceCheckpoint.load(generalUtilities.extractIdForEntity(event)) !== null) return true;
    ...
    */
    let latestNetworkStakerBalanceCheckpointId = protocol.lastNetworkStakerBalanceCheckPoint;
    if (latestNetworkStakerBalanceCheckpointId === null) return false;
    const latestNetworkStakerBalanceCheckpoint = NetworkStakerBalanceCheckpoint.load(latestNetworkStakerBalanceCheckpointId);
    if (latestNetworkStakerBalanceCheckpoint === null || latestNetworkStakerBalanceCheckpoint.blockTime == BigInt.fromI32(0)) return false;

    // Get the date of the network staker balance event candidate and the latest network staker balance checkpoint.
    let dateOfNewNetworkStakerBalanceCheckpoint = new Date(event.block.timestamp.toI32() * 1000);
    let dateOfLatestNetworkStakerBalanceCheckpoint = new Date(latestNetworkStakerBalanceCheckpoint.blockTime.toI32() * 1000);

    // If the latest network staker balance checkpoint and the candidate match in terms of day/month/year, then return false.
    return (
      dateOfNewNetworkStakerBalanceCheckpoint.getUTCFullYear() == dateOfLatestNetworkStakerBalanceCheckpoint.getUTCFullYear() &&
      dateOfNewNetworkStakerBalanceCheckpoint.getUTCMonth() == dateOfLatestNetworkStakerBalanceCheckpoint.getUTCMonth() &&
      dateOfNewNetworkStakerBalanceCheckpoint.getUTCDate() == dateOfLatestNetworkStakerBalanceCheckpoint.getUTCDate()
    );
  }
   ...
   */
  public getTransactionStakers(from: Address, to: Address, blockNumber: BigInt, blockTimeStamp: BigInt): TransactionStakers {
    /*
    ...
    if (fromStaker === null) {
      fromStaker = <Staker>rocketPoolEntityFactory.createStaker(fromId, blockNumber, blockTimeStamp);
    }
    ...
    if (toStaker === null) {
      toStaker = <Staker>rocketPoolEntityFactory.createStaker(toId, blockNumber, blockTimeStamp);
    }
   ...
   */
  public getStakerBalance(staker: Staker, currentRETHExchangeRate: BigInt): StakerBalance {
    let result = new StakerBalance();

    // Determine current rETH & ETH value balance.
    result.currentRETHBalance = staker.rETHBalance;
    if (result.currentRETHBalance < BigInt.fromI32(0)) result.currentRETHBalance = BigInt.fromI32(0);
    result.currentETHBalance = result.currentRETHBalance.times(currentRETHExchangeRate).div(ONE_ETHER_IN_WEI);
    if (result.currentETHBalance < BigInt.fromI32(0)) result.currentETHBalance = BigInt.fromI32(0);
  ...
  public getETHRewardsSincePreviousStakerBalanceCheckpoint(
     ...
     */
    if (previousRETHBalance > BigInt.fromI32(0) && (activeETHBalance > previousETHBalance || activeETHBalance < previousETHBalance)) {
      // CASE #1: The staker his rETH balance stayed the same since last checkpoint.
      if (activeRETHBalance == previousRETHBalance) {
        ethRewardsSincePreviousCheckpoint = activeETHBalance.minus(previousETHBalance);
      }
  ...
  public updateNetworkStakerBalanceCheckpoint(
    ...
    if (rewardsSincePreviousCheckpoint > BigInt.fromI32(0)) {
      ...
      networkCheckpoint.totalStakerETHRewards = networkCheckpoint.totalStakerETHRewards.plus(rewardsSincePreviousCheckpoint);
    }
    ...
    if (staker.rETHBalance > BigInt.fromI32(0)) {
      networkCheckpoint.stakersWithAnRETHBalance = networkCheckpoint.stakersWithAnRETHBalance.plus(BigInt.fromI32(1));
    }
  }
  ...
  public updateNetworkStakerBalanceCheckpointForPreviousCheckpointAndProtocol(
    ...
    if (previousCheckpoint !== null) {
      // Update the total ETH rewards (+/-) up to this checkpoint based on the total rewards.
      networkCheckpoint.totalStakerETHRewards = networkCheckpoint.totalStakerETHRewards.plus(previousCheckpoint.totalStakerETHRewards);
    }
