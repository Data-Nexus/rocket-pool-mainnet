import {PricesUpdated, PricesUpdated1 as PricesUpdatedAtlas} from "../../generated/rocketNetworkPrices/rocketNetworkPrices";
...
import {Node, NetworkNodeBalanceCheckpoint, RocketPoolProtocol} from "../../generated/schema";
...
export function handlePricesUpdated(event: PricesUpdated): void {
  ...
  if (nodeUtilities.hasNetworkNodeBalanceCheckpointHasBeenIndexed(protocol, event)) return;
  ...
  let effectiveRPLBoundsNewMinipool = getEffectiveMinipoolRPLBounds(event.block.number, event.params.rplPrice);
  ...
  nodeUtilities.coerceRunningTotalsBasedOnPreviousCheckpoint(<NetworkNodeBalanceCheckpoint>checkpoint, previousCheckpoint);

  // Update certain totals/averages based on minipool metadata.
  nodeUtilities.updateNetworkNodeBalanceCheckpointForMinipoolMetadata(<NetworkNodeBalanceCheckpoint>checkpoint, metadata.minipoolMetadata);
  nodeUtilities.updateNetworkNodeBalanceCheckpointForRPLMetadata(<NetworkNodeBalanceCheckpoint>checkpoint, metadata.rplMetadata);
  ...
  if (nodeBalanceCheckpoints.indexOf(checkpoint.id) == -1) nodeBalanceCheckpoints.push(checkpoint.id);
...
function generateNodeBalanceCheckpoints(
  ...
  for (let index = 0; index < nodeIds.length; index++) {
    ...
    nodeUtilities.updateNetworkNodeBalanceCheckpointForNode(networkCheckpoint, <Node>node);

    // We need this to calculate the min/max effective RPL needed for the network.
    nodeUtilities.updateMinipoolMetadataWithNode(networkMetadata.minipoolMetadata, <Node>node);

    // We need this to calculate the average RPL claimed rewards on the network level.
    nodeUtilities.updateRPLMetadataWithNode(networkMetadata.rplMetadata, <Node>node);
 ...
 */
function setAverageRplEthRatio(protocol: RocketPoolProtocol, currentCheckpoint: NetworkNodeBalanceCheckpoint): void {
  // Preliminary null checks.
  if (protocol === null || currentCheckpoint === null || currentCheckpoint.rplPriceInETH == BigInt.fromI32(0)) return;
  ...
  for (let index = 0; index < protocol.networkNodeBalanceCheckpoints.length; index++) {
    ...
    let networkNodeBalanceCheckpointId = <string>protocol.networkNodeBalanceCheckpoints[index];
    if (networkNodeBalanceCheckpointId == null) continue;

    // Load the indexed network node balance checkpoint.
    let activeIterationCheckpoint = NetworkNodeBalanceCheckpoint.load(networkNodeBalanceCheckpointId);
    if (activeIterationCheckpoint === null || activeIterationCheckpoint.rplPriceInETH == BigInt.fromI32(0)) continue;

    // Increment running totals.
    totalCheckpointsWithAnRplPriceInETH = totalCheckpointsWithAnRplPriceInETH.plus(BigDecimal.fromString("1"));
  ...
  if (totalRplPriceInEth > BigDecimal.fromString("0") && totalCheckpointsWithAnRplPriceInETH > BigDecimal.fromString("0")) {
 ...
 */
function getEffectiveMinipoolRPLBounds(blockNumber: BigInt, rplPrice: BigInt): EffectiveMinipoolRPLBounds {
